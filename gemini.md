...
> 3. `agents/skills/Seo-keyword-research-implementation.md` (Search intent, NAP consistency, keyword mapping)
...
Also read `s:\client-projects\Hindustan-Electricals\hindustan-motor-mobile\AIRULE.md`.

All color tokens, semantic palettes, and status tones are defined in `src/constants/colors.ts`. Always use tokens from `src/constants/colors.ts` and never hardcode hex values or invent colors on the fly.